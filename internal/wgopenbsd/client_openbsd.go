...
func (c *Client) Devices(ctx context.Context) ([]*wgtypes.Device, error) {
	...
	for _, ifgr := range ifgrs {
		...
		d, err := c.Device(ctx, string(bytes.TrimRight(ifgr.Ifgrqu[:], "\x00")))
		if err != nil {
			return nil, err
		}
...
func (c *Client) Device(_ context.Context, name string) (*wgtypes.Device, error) {
...
func (c *Client) ConfigureDevice(ctx context.Context, name string, cfg wgtypes.Config) error {
	...
	if _, err := c.Device(ctx, name); err != nil {
		return err
	}
