...
// testDial connects to a named pipe without ownership verification,
// suitable for tests where the pipe is not owned by LocalSystem.
func testDial(_ context.Context, device string) (net.Conn, error) {
	return (&namedpipe.DialConfig{}).DialTimeout(device, time.Duration(0))
}
