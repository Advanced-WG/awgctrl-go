...
func TestClientDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dev, err := c.Device(context.Background(), tt.device)
			if err != nil {
...
func testClient(t *testing.T, res []byte) (*Client, func() []byte) {
	...
	c := &Client{
		...
		// Use testDial instead of production dial to avoid ownership
		// checks that fail when the pipe is not owned by LocalSystem.
		find: testFind(dir),
		dial: testDial,
	}
